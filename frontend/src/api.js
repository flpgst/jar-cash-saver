import { differenceInMonths } from "date-fn"
...
function createJar(name, targetValue, color = 'orange', dueDate = null, shared = false) {
    const account = getAccount()
    const jars = getJars()
    const availableFraction = calculateAvailableFraction(jars)
    const fraction = dueDate != null ? calculateFraction() : availableFraction
...
function calculateFraction(targetValue, monthlySaving, dueDate) {
    ...
    return valuePerMonth / account.monthlySaving
}
    ...
    return [
        {
            ...
            description: 'Recebido por criar sua conta no aplicativo e começar a poupar',
            coins: 50,
        }
    ]
}
